		...
		<div class="component commits">
			...
			<?php $n=0; foreach ($post->findCommits() as $commit): ?>
				...
				<li>
					...
					<a href="<?php echo gb_admin::$url ?>helpers/checkout.php?rev=<?php echo $commit->id ?>&amp;paths[]=<?php echo urlencode($post->name) ?>&amp;continue=<?=urlencode(gb::url())?>&amp;style=replace" 
						<?php if ($n === 0): ?>
					...
					<?php if ($n): ?>
					<a href="<?php echo gb_admin::$url ?>helpers/checkout.php?rev=<?php echo $commit->id ?>&amp;paths[]=<?php echo urlencode($post->name) ?>&amp;continue=<?=urlencode(gb::url())?>&amp;style=merge" 
